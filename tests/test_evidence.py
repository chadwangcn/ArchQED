...
class EvidenceTests(unittest.TestCase):
    def setUp(self):
        ...
        init_project(self.root, "demo", "discovery")
        ...
        write_json(self.path, {
            ...
            "source_refs": [],
            "dependencies": [],
            "acceptance": [{"id": "AC-1", "command": "python -c \"print('real')\""}],
            "forbidden": [{"id": "no-placeholder", "pattern": "PLACEHOLDER", "paths": ["src"]}],
        })
    ...
    def test_only_evidence_marks_verified(self):
        ...
        transition_task(self.root, "TASK-1", "in_progress")
        transition_task(self.root, "TASK-1", "implemented_unverified")
        with self.assertRaises(GateError):
            transition_task(self.root, "TASK-1", "verified")
        result = verify_task(self.root, "TASK-1")
        self.assertTrue(result["passed"])
        self.assertEqual(read_json(self.path)["status"], "verified")
    ...
    def test_forbidden_fails_verification(self):
        (self.root / "src").mkdir()
        (self.root / "src/bad.py").write_text("PLACEHOLDER = True\n")
        transition_task(self.root, "TASK-1", "in_progress")
        transition_task(self.root, "TASK-1", "implemented_unverified")
        result = verify_task(self.root, "TASK-1")
        self.assertFalse(result["passed"])
        self.assertEqual(result["forbidden_findings"], 1)
