...
class ChangeTests(unittest.TestCase):
    ...
    def test_repeated_edit_supersedes_pending(self):
        temp, root = self.project()
        self.addCleanup(temp.cleanup)
        path = root / "docs/architecture/system.md"
        path.write_text("ARCH-SYSTEM-001\nOne.\n")
    ...
    def test_stale_approval_rejected(self):
        temp, root = self.project("stabilizing")
        self.addCleanup(temp.cleanup)
        path = root / "docs/architecture/system.md"
        ...
        with self.assertRaises(DriftError):
            approve_change(root, first["id"])
        second = sync_project(root)
        approve_change(root, second["id"])
        complete_compile(root, second["id"])

    def test_feature_change_scopes_by_ref(self):
        temp, root = self.project("stabilizing")
        self.addCleanup(temp.cleanup)
        feature = root / "docs/features/story.md"
        feature.write_text("REQ-STORY-001\nInitial.\n")
        baseline = sync_project(root)
        complete_compile(root, baseline["id"])
